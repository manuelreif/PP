...
2. You have item parameter estimates (e.g. difficulty parameters) of these items from (perhaps from a former application of these items)
...
Now we explore the item response data regarding full or zero score, missing values etc. . Consindering this information, we are able to decide which estimation method we will apply.
...
#(in this case this is simply a computational issue, because the right estimates are plugged in as a last step.)

We use the `PP_4pl()` function for our estimation. We decide to apply a common maximum likelihood estimation, and do not remove duplicated response pattern from estimation because there are only `r sum(rdup)` duplicated pattern. If the data set were much larger, duplicated pattern are more likely and therefore choosing to remove these pattern would speed up the estimation process significantly. (Choosing this option or not, does not change the numerical results!)
...
1. The function returns the **point estimators** and standard errors for each person. Whatever the (valid) input was, the main result is always an matrix with two columns and a number of rows which equals the number of persons (`res1plmle$resPP$resPP`).
